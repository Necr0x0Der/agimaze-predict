...
def greedy_target_bytes(model: VisualTransformer, example: PreparedExample) -> bytes:
    ...
    for _ in range(expected_length):
        ...
        if model.config.pos_readout == "full_text":
            ...
            # Query-final '>' predicts the first answer byte.  Thereafter the
            # decoder receives every previously generated answer byte, exactly
            # as it receives every preceding target byte under teacher forcing.
            # (Do not exclude the final generated byte: that would repeatedly
            # query the distribution for the prior position.)
            target_input = torch.tensor(
                [[prefix[-1], *generated[item.target_start:]]], dtype=torch.long, device=device
            )
            logits = model(
                input_ids[:, : item.target_start],
                visual_maps=visual_maps,
                event_positions=events,
                event_counts=event_counts[:, : item.target_start],

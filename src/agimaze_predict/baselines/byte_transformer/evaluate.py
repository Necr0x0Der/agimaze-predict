...
def greedy_target_bytes(model: ByteTransformer, prefix: Sequence[int], *, target_bytes: int) -> bytes:
    ...
    for _ in range(target_bytes):
        ...
        aux_lengths = (
            torch.tensor([len(prefix)], dtype=torch.long, device=input_ids.device)
            if model.config.aux_latents_per_token
            else None
        )
    ...
    *,
    ...
    for start in range(0, len(examples), batch_size):
        ...
        logits, diagnostics = model(
            input_ids,
            aux_lengths=aux_lengths if model.config.aux_latents_per_token else None,
    ...
    if model.config.aux_latents_per_token:
        ...
        metrics.update(
            {key: value / diagnostics_batches for key, value in diagnostics_total.items()}
        )
    if greedy_error_log is not None:
